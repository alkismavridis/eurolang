...
/*
	This is called by the backend, to tell us to start compiling
*/
static void eul_langhook_parse_file (void) {
  fprintf(stderr, "Hello mojo!\n");
}
...
tiny_langhook_type_for_mode (enum machine_mode mode, int unsignedp)
{
  ...
  if (mode == TYPE_MODE (long_long_integer_type_node))
	...
	if (COMPLEX_MODE_P (mode)) {
	  if (mode == TYPE_MODE (complex_float_type_node)) return complex_float_type_node;
	  if (mode == TYPE_MODE (complex_double_type_node)) return complex_double_type_node;
	  if (mode == TYPE_MODE (complex_long_double_type_node)) return complex_long_double_type_node;
	  if (mode == TYPE_MODE (complex_integer_type_node) && !unsignedp) return complex_integer_type_node;
	}
...
#define LANG_HOOKS_PARSE_FILE eul_langhook_parse_file
